...
CanController.prototype = {
  ...
  _isDatabaseServiceCreated: function () {
    if (this.databaseService === undefined) {
      console.error(
        "error adding listener as databaseService is not created. Run _initializeCanConnection to create."
      );
      return false;
    }
    return true;
  ...
  _initializeCanConnection: function (database, socket, bus) {
    ...
    try {
      ...
      console.error(
        "error %s when creating channel for socket %s",
        e.message,
        socket
      );
      return;
    }
    this.databaseService = new can.DatabaseService(
      self.channel,
      network.buses[bus]
    );
    self.channel.start();
  ...
  registerListener: function (message, signal, update, callback) {
    var self = this;
    if (!self._isDatabaseServiceCreated()) return;
    ...
    if (update) {
      ...
      self.databaseService.messages[message].signals[signal].onUpdate(function (
        sig
      ) {
      ...
      self.databaseService.messages[message].signals[signal].onChange(function (
        sig
      ) {
        callback(message, sig);
      });
  ...
  sendCanMessage: function (message) {
    ...
    if (!this._isDatabaseServiceCreated()) return;
    try {
  ...
  addRetainedCanMessage: function (message) {
    ...
    if (self.databaseService.messages[message].interval) {
      //interval is provided via kcd file
      var intervalId = setInterval(function () {
        self.sendCanMessage(message);
      }, self.databaseService.messages[message].interval);
    } else {
      //utilize the default 20 ms cycle rate
      var intervalId = setInterval(function () {
        self.sendCanMessage(message);
      }, self.refreshRate);
    }
  ...
  removeRetainedCanMessage: function (message) {
    ...
    if (!self.retainedCanMessages.has(message)) return;
    ...
    if (self.databaseService.messages[message]) {
      if (signal === undefined || signal === "") {
        ...
        if (self.databaseService.messages[message].signals[signal]) {
          foundValue =
            self.databaseService.messages[message].signals[signal].value;
        }
      }
    }
